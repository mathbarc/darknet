...
void validate_yolo(char* cfgfile, char* weightfile)
{
    ...
    for (i = nthreads; i < m + nthreads; i += nthreads) {
        ...
        for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
            ...
            if (nms)
                do_nms_sort_v2(boxes, probs, l.side * l.side * l.n, classes, iou_thresh);
...
void run_yolo(int argc, char** argv)
{
    ...
    if (0 == strcmp(argv[2], "test"))
        ...
        demo(cfg, weights, thresh, hier_thresh, cam_index, filename, voc_names, 20, 1, frame_skip, prefix, out_filename, mjpeg_port, 0, json_port, dont_show, ext_output, 0, 0, 0, 0, 0, json_file_output);
}
