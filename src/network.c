        ...
        case SGDR:
            {
                ...
                rate = net.learning_rate_min + 0.5 * (net.learning_rate - net.learning_rate_min) * (1. + cos((float)(batch_num - last_iteration_start) * 3.14159265 / cycle_size));
...
float train_network_waitkey(network net, data d, int wait_key)
{
    ...
    if ((*net.cur_iteration) < reject_stop_point && net.weights_reject_freq && (*net.cur_iteration) % net.weights_reject_freq == 0) {
    ...
    for (i = 0; i < nboxes; ++i) {
        for (j = 0; j < classes; ++j) {
            ...
            if (dets[i].prob[j] > thresh && show) {
                ...
                sprintf(buf, "  {\"class_id\":%d, \"name\":\"%s\", \"relative_coordinates\":{\"center_x\":%f, \"center_y\":%f, \"width\":%f, \"height\":%f}, \"confidence\":%f}", j, names[j], dets[i].bbox.x, dets[i].bbox.y, dets[i].bbox.w, dets[i].bbox.h, dets[i].prob[j]);
