...
uint32_t reverse_32_bit(uint32_t a)
{
    ...
    return (reverse_8_bit(a >> 24) << 0) | (reverse_8_bit(a >> 16) << 8) | (reverse_8_bit(a >> 8) << 16) | (reverse_8_bit(a >> 0) << 24);
}
...
{
    return ((a & 0x1) << 7) | ((a & 0x2) << 5) | ((a & 0x4) << 3) | ((a & 0x8) << 1) | ((a & 0x10) >> 1) | ((a & 0x20) >> 3) | ((a & 0x40) >> 5) | ((a & 0x80) >> 7);
}
...
void transpose8rS32_reversed_diagonale(unsigned char* A, unsigned char* B, int m, int n)
{
    ...
    B[7 * n] = reverse_byte(x >> 24);
    B[6 * n] = reverse_byte(x >> 16);
    B[5 * n] = reverse_byte(x >> 8);
    B[4 * n] = reverse_byte(x);
    B[3 * n] = reverse_byte(y >> 24);
    B[2 * n] = reverse_byte(y >> 16);
    B[1 * n] = reverse_byte(y >> 8);
    B[0 * n] = reverse_byte(y);
}
...
void convolution_2d_old(int w, int h, int ksize, int n, int c, int pad, int stride, float* weights, float* input, float* output)
{
                ...
                for (x = 0; x < w; ++x) {
                    ...
                    for (f_y = 0; f_y < ksize; ++f_y) {
                        ...
                        for (f_x = 0; f_x < ksize; ++f_x) {
                            ...
                            if (input_y < 0 || input_x < 0 || input_y >= h || input_x >= w)
                                continue;
...
void convolution_2d(int w, int h, int ksize, int n, int c, int pad, int stride, float* weights, float* input, float* output, float* mean)
{
    ...
    __m256i all256_last_zero = _mm256_set_epi32(0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0x0);
        ...
        for (c = 0; c < channels_col; ++c) {
            ...
            for (h = pad; h < height_col - pad; ++h) {
                ...
                for (; w < width_col - pad; ++w) {
                    ...
                    if (val > 0)
                        set_bit((unsigned char* const)data_col, col_index);
                }
            }
        ...
        for (c = 0; c < channels_col; ++c) {
            ...
            for (h = pad; h < height_col - pad; ++h) {
                for (w = pad; w < width_col - pad - 8; w += 1) {
                    ...
                    float val = data_im[im_col + width * (im_row + height * c_im)];
                    if (val > 0)
                        set_bit((unsigned char*)data_col, col_index);
                }
