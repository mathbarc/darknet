...
void train_classifier(char* datacfg, char* cfgfile, char* weightfile, int* gpus, int ngpus, int clear, int dontuse_opencv, int dont_show, int mjpeg_port, int calc_topk, int show_imgs, char* chart_path)
{
    ...
    while (get_current_batch(net) < net.max_batches || net.max_batches == 0) {
        ...
        if (!dontuse_opencv)
            draw_train_loss(windows_name, img, img_size, avg_loss, max_img_loss, i, net.max_batches, topk, draw_precision, topk_buff, avg_contrastive_acc / 100, dont_show, mjpeg_port, avg_time);
#endif // OPENCV
...
void threat_classifier(char* datacfg, char* cfgfile, char* weightfile, int cam_index, const char* filename)
{
    ...
    while (1) {
        ...
        if (threat > .97) {
            draw_box_width(out, x2 + .5 * w + border, y1 + .02 * h - 2 * border, x2 + .5 * w + 6 * border, y1 + .02 * h + 3 * border, 3 * border, 1, 0, 0);
        }
        draw_box_width(out, x2 + .5 * w + border, y1 + .02 * h - 2 * border, x2 + .5 * w + 6 * border, y1 + .02 * h + 3 * border, .5 * border, 0, 0, 0);
        draw_box_width(out, x2 + border, y1 + .42 * h, x2 + .5 * w, y1 + .42 * h + border, border, 0, 0, 0);
        if (threat > .57) {
            draw_box_width(out, x2 + .5 * w + border, y1 + .42 * h - 2 * border, x2 + .5 * w + 6 * border, y1 + .42 * h + 3 * border, 3 * border, 1, 1, 0);
        }
        draw_box_width(out, x2 + .5 * w + border, y1 + .42 * h - 2 * border, x2 + .5 * w + 6 * border, y1 + .42 * h + 3 * border, .5 * border, 0, 0, 0);
...
void run_classifier(int argc, char** argv)
{
    ...
    if (0 == strcmp(argv[2], "predict"))
        ...
        train_classifier(data, cfg, weights, gpus, ngpus, clear, dontuse_opencv, dont_show, mjpeg_port, calc_topk, show_imgs, chart_path);
