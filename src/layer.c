...
void free_layer_custom(layer l, int keep_cudnn_desc)
{
    ...
    if (l.cost)
        free(l.cost);
    if (l.labels && !l.detection)
        free(l.labels);
    if (l.class_ids && !l.detection)
        free(l.class_ids);
    if (l.cos_sim)
        free(l.cos_sim);
    if (l.exp_cos_sim)
        free(l.exp_cos_sim);
    if (l.p_constrastive)
        free(l.p_constrastive);
    if (l.embedding_output)
        free(l.embedding_output);
    if (l.state)
        free(l.state);
    if (l.prev_state)
        free(l.prev_state);
    ...
    if (l.biases_ema)
        free(l.biases_ema), l.biases_ema = NULL;
    if (l.scales_ema)
        free(l.scales_ema), l.scales_ema = NULL;
    if (l.weights_ema)
        free(l.weights_ema), l.weights_ema = NULL;
    if (l.weights)
    ...
    if (l.contrast_p_gpu)
        cuda_free((float*)l.contrast_p_gpu);
    if (l.z_gpu)
        cuda_free(l.z_gpu);
    if (l.r_gpu)
        cuda_free(l.r_gpu);
    ...
    if (l.optimized_memory < 2) {
        ...
        if (l.output_avg_gpu)
            cuda_free(l.output_avg_gpu), l.output_avg_gpu = NULL;
    ...
    if (l.h_gpu)
        cuda_free(l.h_gpu);
    if (l.bottelneck_hi_gpu)
        cuda_free(l.bottelneck_hi_gpu);
    if (l.bottelneck_delta_gpu)
