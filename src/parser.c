...
local_layer parse_local(list* options, size_params params)
{
    ...
    batch = params.batch;
    if (!(h && w && c))
        error("Layer before local layer must output image.", DARKNET_LOC);
...
crop_layer parse_crop(list* options, size_params params)
{
    ...
    batch = params.batch;
    if (!(h && w && c))
        error("Layer before crop layer must output image.", DARKNET_LOC);
...
layer parse_reorg(list* options, size_params params)
{
    ...
    batch = params.batch;
    if (!(h && w && c))
        error("Layer before reorg layer must output image.", DARKNET_LOC);
...
maxpool_layer parse_local_avgpool(list* options, size_params params)
{
    ...
    batch = params.batch;
    if (!(h && w && c))
        error("Layer before [local_avgpool] layer must output image.", DARKNET_LOC);
...
maxpool_layer parse_maxpool(list* options, size_params params)
{
    ...
    batch = params.batch;
    if (!(h && w && c))
        error("Layer before [maxpool] layer must output image.", DARKNET_LOC);
...
avgpool_layer parse_avgpool(list* options, size_params params)
{
    ...
    batch = params.batch;
    if (!(h && w && c))
        error("Layer before avgpool layer must output image.", DARKNET_LOC);
...
layer parse_shortcut(list* options, size_params params, network net)
{
    ...
    int len = strlen(l);
    if (!l)
        error("Route Layer must specify input layers: from = ...", DARKNET_LOC);
    int n = 1;
    int i;
    for (i = 0; i < len; ++i) {
        if (l[i] == ',')
    ...
    layer s = make_shortcut_layer(params.batch, n, layers, sizes, params.w, params.h, params.c, layers_output, layers_delta, layers_output_gpu, layers_delta_gpu, weights_type, weights_normalization, activation, params.train);
...
route_layer parse_route(list* options, size_params params)
{
    ...
    if (!l)
        error("Route Layer must specify input layers", DARKNET_LOC);
    int len = strlen(l);
    int n = 1;
    int i;
    for (i = 0; i < len; ++i) {
...
void parse_net_options(list* options, network* net)
{
    ...
    *net->cuda_graph_ready = 0;
    ...
    if (!net->inputs && !(net->h && net->w && net->c))
        error("No input parameters supplied", DARKNET_LOC);
    ...
    if (net->policy == STEP) {
        ...
        if (net->policy == STEPS && (!l || !p))
            error("STEPS policy must have steps and scales in cfg file", DARKNET_LOC);
...
void set_train_only_bn(network net)
{
    ...
    for (i = net.n - 1; i >= 0; --i) {
        if (net.layers[i].train_only_bn)
            train_only_bn = net.layers[i].train_only_bn; // set l.train_only_bn for all previous layers
        if (train_only_bn) {
