...
void wait_stream(int i)
{
    ...
    if (event_counter >= max_events)
        error("CUDA max_events exceeded", DARKNET_LOC);
...
void pre_allocate_pinned_memory(const size_t size)
{
    ...
    if (!pinned_ptr) {
        ...
        if (!pinned_ptr)
            error("calloc failed in pre_allocate()", DARKNET_LOC);
        ...
        for (k = 0; k < num_of_blocks; ++k) {
            ...
            CHECK_CUDA(status);
            if (!pinned_ptr[k])
                error("cudaHostAlloc failed", DARKNET_LOC);
            else {
...
float* cuda_make_array(float* x, size_t n)
{
    ...
    if (!x_gpu)
        error("Cuda malloc failed", DARKNET_LOC);
    return x_gpu;
}
