...
void resize_deconvolutional_layer(deconvolutional_layer* l, int h, int w)
{
    ...
    l->col_image = (float*)xrealloc(l->col_image, out_h * out_w * l->size * l->size * l->c * sizeof(float));
    l->output = (float*)xrealloc(l->output, l->batch * out_h * out_w * l->n * sizeof(float));
    l->delta = (float*)xrealloc(l->delta, l->batch * out_h * out_w * l->n * sizeof(float));
#ifdef GPU
