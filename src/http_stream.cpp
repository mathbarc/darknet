    ...
    bool write(char const* outputbuf)
    {
        ...
        for (int s = 0; s <= maxfd; s++) {
            ...
            if (s == sock) // request on master socket, accept and send main header.
            {
                ...
                _write(client, "Server: Mozarella/2.2\r\n"
                               "Accept-Range: bytes\r\n"
                               "Connection: close\r\n"
                               "Max-Age: 0\r\n"
                               "Expires: 0\r\n"
...
struct detection_t : detection {
    ...
    {
        if (embeddings)
            embeddings = make_float_array(det.embeddings, embedding_size);
        if (prob)
            prob = make_float_array(det.prob, classes);
        if (uc)
            uc = make_float_array(det.uc, 4);
    }
