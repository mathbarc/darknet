...
void forward_contrastive_layer(contrastive_layer l, network_state state)
{
                        ...
                        for (n2 = 0; n2 < l.n; ++n2) {
                            for (h2 = 0; h2 < l.h; ++h2) {
                                for (w2 = 0; w2 < l.w; ++w2) {
                                    ...
                                    // calc good sim
                                    if (l.labels[z_index] == l.labels[z_index2] && max_sim_same[z_index] < sim)
                                        max_sim_same[z_index] = sim;
                                    if (l.labels[z_index] != l.labels[z_index2] && max_sim_diff[z_index] < sim)
    ...
    for (i = 0; i < l.batch * l.inputs; ++i) {
        if (max_sim_same[i] >= -1 && max_sim_diff[i] >= -1) {
            ...
            if (max_sim_diff[i] < max_sim_same[i])
                good_sims++;
        }
    }
    if (all_sims > 0) {
